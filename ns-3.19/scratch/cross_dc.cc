...
int main(int argc, char *argv[]) {
...
#endif
    {
        ...
        while (!conf.eof()) {
            ...
            } else if (key.compare("DCI_SWITCH_IDS") == 0) {
                int n_dci;
                conf >> n_dci;
                std::cerr << "DCI_SWITCH_IDS\t\t\t\t";
                for (int i = 0; i < n_dci; i++) {
                    uint32_t id;
                    conf >> id;
                    dci_switch_ids.push_back(id);
                    std::cerr << ' ' << id;
                }
                std::cerr << '\n';
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() == 1) {  // is switch
            ...
            // check if the switch is a dci-switch
            ...
            // set different buffer size for different switch types
            if (is_dci_switch) {
                sw->m_mmu->ConfigBufferSize(dci_buffer_size * 1024 * 1024);
                std::cout << "Switch " << sw->GetId() << " configured as DCI-Switch with buffer size: " 
                        << dci_buffer_size << "MB" << std::endl;
            } else {
                sw->m_mmu->ConfigBufferSize(buffer_size * 1024 * 1024);
            }  // default 0, specify in run.py!!
    ...
    // 添加跨数据中心拓扑
    topo2bdpMap[std::string("cross_dc_k4_dc2_os2")] = 208000;  // cross-dc -> 100G internal, 400G DCI
    topo2bdpMap[std::string("cross_dc_k4_dc3_os2")] = 312000;  // cross-dc -> 100G internal, 400G DCI
